...
refdatadir = os.path.join(data_dir, 'speechdb/aochildes_2021/CHILDES_ao_dataset_1word')
source_basename = os.path.join(data_dir, 'rundata/GILES_CogSci/txt_out/agetests_cogsci_final_1word/GILES_CHILDES_agecond_final_1word_5layers_512emb_512ff_8head_8000voc_5do_500k_100t_')
outputpath = os.path.join(data_dir, 'rundata/GILES_CogSci/overlaps_500k_1word.csv')
...
outputpath_newwords = os.path.join(data_dir, 'rundata/GILES_CogSci/newwords')
os.remove(os.path.join(outputpath_newwords, 'uttlen1.txt'))
os.remove(os.path.join(outputpath_newwords, 'uttlen4.txt'))
os.remove(os.path.join(outputpath_newwords, 'uttlen8.txt'))
...
for source_file in file_list:
    ...
    if(sum(ages_to_analyze == agebin)):
        with open(os.path.join(outputpath_newwords, 'uttlen1.txt'), 'a', newline='') as file:
            file.write("{}".format(agebin) + ' months \n')
        with open(os.path.join(outputpath_newwords, 'uttlen4.txt'), 'a', newline='') as file:
            file.write("{}".format(agebin) + ' months \n')
        with open(os.path.join(outputpath_newwords, 'uttlen8.txt'), 'a', newline='') as file:
            file.write("{}".format(agebin) + ' months \n')
        ...
        for uttlen in range(1,np.max(N_w)):
            ...
            print(f"Length {uttlen}: {prop_uq[uttlen]*100:.2f}% (N = {count_uq[uttlen]:.0f})")
            UL.append(uttlen)
            UQ.append(prop_uq[uttlen]*100)
            UC.append(count_uq[uttlen])
            UA.append(agebin)
        print(f"Overall: {prop_unique:.2f} ({agebin} mo)")
...
outputpath = os.path.join(data_dir, 'rundata/GILES_CogSci/overlaps_CHILDES_1word_fixed.csv')
refdatadir = os.path.join(data_dir, 'speechdb/aochildes_2021/CHILDES_ao_dataset_1word')
...
for i in range(0,len(ages_to_test)):
    source_basename = os.path.join(data_dir, f'rundata/GILES_CogSci/reference_data_CHILDES_1word/childes_age_0{ages_to_test[i]:02d}/')
    pattern = source_basename + '*.txt'
    file_list = glob.glob(pattern)
    file_list.sort()
        ...
        if (sum(ages_to_analyze == agebin)):
            ...
            for i in range(N):
                ...
                if count == 1 and len(string_to_search.split()) == 1:
                    print(string_to_search)
                    with open(os.path.join(outputpath_newwords, 'uttlen1_CHILDES.txt'), 'a', newline='') as file:
                        file.write(string_to_search + '\n')
                if count == 1 and len(string_to_search.split()) == 4:
                    print(string_to_search)
                    with open(os.path.join(outputpath_newwords, 'uttlen4_CHILDES.txt'), 'a', newline='') as file:
                        file.write(string_to_search + '\n')
                if count == 1 and len(string_to_search.split()) == 8:
                    print(string_to_search)
                    with open(os.path.join(outputpath_newwords, 'uttlen8_CHILDES.txt'), 'a', newline='') as file:
                        file.write(string_to_search + '\n')
