...
home_dir = '/Users/HOME_DIR'
if os.path.isdir(home_dir):
    maindir = f'{home_dir}/Documents/koodit/dev/GILES_alpha/'       # Location of the code (where to save models etc.)
    datadir = f"{home_dir}/speechdb/aochildes_2021/CHILDES_ao_dataset_1word/"     # Where CHILDES_AO .txt files are located?
    log_dir = f"{home_dir}/rundata/logs/fit/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") # Where to log training?
else:
    maindir = f'{home_dir}/tools/GILES_alpha/'
    datadir = f"{home_dir}/data/CHILDES_ao_dataset_1word/"
    log_dir = f"{home_dir}/logs/fit/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
