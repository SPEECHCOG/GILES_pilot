...
pos_to_analyze = ('ADJ', 'ADV', 'INTJ', 'NOUN', 'PROPN', 'VERB',                 # Open class words
                  'ADP', 'AUX', 'CONJ', 'DET', 'NUM', 'PART', 'PRON')  # Close class words: 'CCONJ', 'SCONJ'


def parse_args(parser):
    # Pipeline flags
    parser.add_argument('--sample-data', action='store_true', default=True, help='Sample the data.')
    parser.add_argument('--analyse-data', action='store_true', default=True, help='Analyse the data samples.')
    parser.add_argument('--recalculate-features', action='store_true', default=False, help='Recalculate existing figures.')
    parser.add_argument('--update-features', action='store_true', default=False, help='Update feature measures.')
    parser.add_argument('--features-to-update', nargs='+',
                        default=['sent_length', "phrase_per_sent", "verb_per_sent", "nom_sent_rate",
                                 "intj_sent_rate", "complex_sent_rate", "mean_dep_distance", "mean_dep_per_token",
                                 "synt_tree_height"],
                        help='Names of the measures to be recalculated')
    parser.add_argument('--statistical-tests', action='store_true', default=False, help='Perform statistical tests.')
    parser.add_argument('--plot-figures', action='store_true', default=True, help='Plot figures.')
    ...
    parser.add_argument('--violinplot-with-quartiles', action='store_true', default=True, help='Draw quartiles in violin plots.')

    # Paths to directories and files
    parser.add_argument("--input-path", type=str, default=os.path.join('..', 'data', 'aochildes.2021', 'CHILDES_ao_dataset'), help="Input directory or file path.")
    parser.add_argument("--output-path", type=str, default=os.path.join('output', 'giles'), help="Output directory path.")
    parser.add_argument('--analysis-parameter-file', default='analysis_parameters_text.json', help='JSON with analysis parameters')

    # Other parameters
    parser.add_argument('--random-seed', type=int, default=7, help="Random seed.")
    parser.add_argument('--reference-dataset', default='childes_age_057', help='Reference dataset name')
    parser.add_argument('--analytics-required', default='', help='The name of the analytical measure to be tested')
    parser.add_argument('--token-count-limit', type=int, default=0, help='Minimal number of tokens for a word to be counted')
    parser.add_argument('--sentence-max-length', type=int, default=0, help='The maximum length of a sentence (in words). Longer sentences will not be loaded. 0 - for no limitation.')
    parser.add_argument('--dataset-min-size', type=int, default=2000, help='The minimum size of a dataset (in sentences). Smaller datasets will not be loaded. 0 - for no limitation.')
    parser.add_argument('--sampling-size', type=int, default=100, help='Size of sampling distribution to be compared')
    parser.add_argument('--sampling-tokens', type=int, default=10000, help='Sampling: number of tokens for token-wise comparison')
    parser.add_argument('--sampling-sentences', type=int, default=1000, help='Sampling: number of sentences for sentence-wise comparison')
    parser.add_argument('--sampling-batches_n', type=int, default=100, help='Sampling: number of batches for perplexity calculation')
    parser.add_argument('--sampling-batches_size', type=int, default=50, help='Sampling: size of batches for perplexity calculation')
    return parser.parse_args()


parser = ArgumentParser(description='Calculate measures of language complexity to compare datasets.',
                        allow_abbrev=False)
...
def main():
    ...
    figure_dir = Path(output_path, f'img_childes')
    sampling_dir = Path(output_path, f'samples')
    feature_file = Path(report_dir, f'features_childes.txt')
    ...
    if args.analyse_data:
        ...
        for ds in sorted(dataset_samples):
            print(f'\n{ds}')
            ds_report_path = Path(report_dir, f'{ds}.features')
            if not args.recalculate_features and ds_report_path.is_file():
                continue
            for analysis in analytics_required:
                ...
                elif analysis in ['pos_rate', 'pos_ttr']:
    ...
    if args.plot_figures:
        ...
        for analysis in sorted(feature_distributions):
            print(f'--- {analysis}')
            if analysis.startswith('pos_'):
                pos = analysis.split('_')[-1]
                label = '_'.join(analysis.split('_')[:-1])
                plot_title = f"{analysis_setup[label]['plot_title']} - {pos}"
                plotting_style = analysis_setup[label]['plot_style']
            else:
...
def dataset_sampling(datasets: dict):
    sampling_datasets = {}
...
def load_datasets(dataset_path: Path) -> dict[str, dict[str, list[str]]]:
    ...
    if dataset_path.is_file() and dataset_path.suffix == '.txt':
        ...
        ds_files = sorted(dataset_path.glob('*.txt'))
        ds_dirs = sorted([p for p in dataset_path.iterdir() if p.is_dir()])
        for ds_file in ds_files:
            print(ds_file)
            datasets[ds_file.stem]['txt'] = read_dataset_from_file(ds_file)
            ds_parsed = ds_file.with_suffix('.parsed')
            datasets[ds_file.stem]['parsed'] = parse_dataset(datasets[ds_file.stem]['txt'], ds_parsed, ds_file.stem)
...
def sent_len(sentence: Sentence) -> int:
    words = [w for w in sentence.words if w.upos != 'PUNCT']
    return len(words)
...
def _clear_x_tick_label(text: str) -> str:
    text = os.path.split(text)[-1].replace('.samples', '').replace('gpt.model.', '')
    text = text.replace('childes_age_0', '')
    return text
