...
def calc_dep_distance(data: list[list[Sentence]]) -> Union[list[float], list[np.ndarray]]:
    """
    Calculates an average dependency distance in each sentence of the text.
    ...
    for text in data:
        ...
        for sentence in text:
            distances = [abs(sentence.words[i].id - sentence.words[i].head) for i in range(len(sentence.words)) if sentence.words[i].head != 0]
            if len(distances) > 0:
                text_values.append(np.mean(distances))
        feature_measures.append(np.mean(text_values))
    return feature_measures


def calc_dep_per_token(data: list[list[Sentence]]) -> Union[list[float], list[np.ndarray]]:
    """
    Calculates an number of dependencies per token in each sentence of the text.
    It is calculated as a number of tokens divided by the number of heads.
    :param data: list sample texts to be evaluated. Each sample text is a list of Sentence.
    """
    feature_measures = []
    for text in data:
        text_values = []
        for sentence in text:
            dep_per_token = len(sentence.words) / len({w.head for w in sentence.words})
            text_values.append(dep_per_token)
        feature_measures.append(np.mean(text_values))
    return feature_measures


def calc_synt_tree_height(data: list[list[Sentence]]) -> Union[list[float], list[np.ndarray]]:
    """
    Calculates an number of dependencies per token in each sentence of the text.
    It is calculated as a number of tokens divided by the number of heads.
    :param data: list sample texts to be evaluated. Each sample text is a list of Sentence.
...
def calc_pos_rate(data: list[list[Word]], **kwargs) -> Union[dict[str, list[float]], None]:
    ...
    if 'CONJ' in pos_to_analyze:
        feature_measures['CONJ'] = [sample.get('SCONJ', 0) + sample.get('CCONJ', 0) for sample in pos_rate]
    return feature_measures


def calc_pos_ttr(data: list[list[Word]], **kwargs) -> Union[dict[str, list[float]], None]:
    """
    Calculates a TTR of words of various part-of-speech in a given text.
    :param data: list sample texts to be evaluated. Each sample text is a list of Word.
    """
    kwargs = kwargs.get('arg', None)  # get real **kwargs passed into function as 'arg' argument
    pos_to_analyze = None
    ...
    pos_ttr = []
    for text in data:
        wp_words = defaultdict(list)
        for w in text:
            wp_words[w.pos].append(w)
        text_rates = {wp: len({w.lemma for w in wp_words[wp]}) / len(wp_words[wp]) for wp in wp_words}
        pos_ttr.append(text_rates)
    feature_measures = {p: [sample.get(p, 0) for sample in pos_ttr] for p in pos_to_analyze}
    if 'CONJ' in pos_to_analyze:
        feature_measures['CONJ'] = [sample.get('SCONJ', 0) + sample.get('CCONJ', 0) for sample in pos_ttr]
    return feature_measures
...
def calc_root_pos_distribution(data: list[list[Sentence]]) -> dict[str, float]:
    ...
    for text in data:
        ...
        pos_names.update(list(s_root_rate.keys()))
        pos_rate.append(rate)
...
def calc_type_token_ratio(data: list[list[Word, Sentence]]) -> list[float]:
    """
...
def _get_dist_to_root(word, sentence, max_distance=100):
    distance = 0
    while word.head != 0 and distance < max_distance:
        word = sentence.words[word.head-1]  # because the word IDs starts from 1 in Stanza tree.
        distance += 1
    return distance
